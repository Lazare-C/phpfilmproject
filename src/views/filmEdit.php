            ...
            <div class="lg:w-2/5 md:w-1/2 w-2/3">
                <form class="bg-white p-10 rounded-lg shadow-lg min-w-full" method="post"  enctype="multipart/form-data" >
                    ...
                    <div>
                        ...
                        <input required class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none" value="<?php echo $GLOBALS['film']->getNom(); ?>" type="text" name="nom" id="nom" placeholder="Nom du film" />
                    </div>
                    <div>
                        <label class="text-gray-800 font-semibold block my-3 text-md" for="annee">Année</label>
                        <input required class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none" value="<?php echo $GLOBALS['film']->getAnnee(); ?>" type="number" min="1800" name="annee" id="annee" placeholder="Année"  />
                    </div>
                    <div>
                        <label class="text-gray-800 font-semibold block my-3 text-md" for="score">Score</label>
                        <input required class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none" min="0" max="10" value="<?php echo $GLOBALS['film']->getScore(); ?>" type="number" step="0.01" name="score" id="score" placeholder="Score"  />
                    </div>
                    <div>
                        <label class="text-gray-800 font-semibold block my-3 text-md" for="nb_vote">Nombre de Votes</label>
                        <input required class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none" value="<?php echo $GLOBALS['film']->getNbVote(); ?>" type="number" name="nb_vote" id="nb_vote" placeholder="Nombre de Votes"  />
                    </div>
                    <div>
